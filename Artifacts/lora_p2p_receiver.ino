...
float frequency = 868.0f; int spreadingFactor = 7; float bandwidth = 125.0f;
int codingRate = 5; byte syncWord = 0x12; int preambleLength = 8;

void setup() {
  Serial.begin(115200);
  Serial.println("LoRa P2P Receiver Initializing...");
  int state = radio.begin(frequency);
  if (state != RADIOLIB_ERR_NONE) { Serial.print("Init Failed: "); Serial.println(state); while (true); }
  radio.setSpreadingFactor(7); radio.setBandwidth(125); radio.setCodingRate(5);
  radio.setSyncWord(0x12); radio.setPreambleLength(8);
  Serial.println("[LoRa P2P] Starting receive...");
}

void loop() {
  byte byteArr[256]; int state = radio.receive(byteArr, 0); int len = radio.getPacketLength();
  if (state == RADIOLIB_ERR_NONE) {
    Serial.print("[LoRa P2P] RX OK! RSSI:"); Serial.print(radio.getRSSI()); Serial.print(" SNR:"); Serial.print(radio.getSNR()); Serial.print(" Len:"); Serial.print(len);
    Serial.print(" Data:'"); for(int i=0; i<len; i++) Serial.print((char)byteArr[i]); Serial.println("'");
    
  } else if (state == RADIOLIB_ERR_CRC_MISMATCH) Serial.println("[LoRa P2P] CRC Error!");
  // else if (state != RADIOLIB_ERR_RX_TIMEOUT) { Serial.print("[LoRa P2P] RX Fail:"); Serial.println(state); }
}
